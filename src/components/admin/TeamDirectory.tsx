      ...
      <div className="admin-stack">
        ...
        {canManageTeam ? (
          <section className="panel">
            ...
            <form className="toolbar" onSubmit={(event) => void createMember(event)} style={{ flexWrap: "wrap", gap: "12px" }}>
              ...
              <label className="search-field" style={{ minWidth: "180px" }}>
                <select value={role} aria-label="Staff role" onChange={(event) => setRole(event.target.value as typeof role)}>
                  {TENANT_ASSIGNABLE_ROLES.map((value) => (
                    <option key={value} value={value}>{formatStaffRole(value)}</option>
                  ))}
                </select>
              </label>
              <button type="submit" className="filter-chip filter-chip-active" disabled={busy}>
                Add member
              </button>
            </form>
          </section>
                    ...
                    <td>
                      <div className="person-cell">
                        ...
                        <div>
                          ...
                          <div className="muted mono" style={{ fontSize: "0.8rem" }}>{user.id}</div>
                        </div>
                      </div>
                    </td>
                    <td>
